# 配置管理系统

<cite>
**本文档引用的文件**
- [sparsify/config.py](file://sparsify/config.py)
- [sparsify/sparse_coder.py](file://sparsify/sparse_coder.py)
- [sparsify/trainer.py](file://sparsify/trainer.py)
- [sparsify/device.py](file://sparsify/device.py)
- [sparsify/checkpoint.py](file://sparsify/checkpoint.py)
- [sparsify/tiled_sparse_coder.py](file://sparsify/tiled_sparse_coder.py)
- [sparsify/utils.py](file://sparsify/utils.py)
- [sparsify/__main__.py](file://sparsify/__main__.py)
- [docs/training/config-reference.md](file://docs/training/config-reference.md)
- [sparsify/jumprelu_sparse_coder.py](file://sparsify/jumprelu_sparse_coder.py)
- [sparsify/group_topk_sparse_coder.py](file://sparsify/group_topk_sparse_coder.py)
- [sparsify/gated_sparse_coder.py](file://sparsify/gated_sparse_coder.py)
- [thresholds/Qwen3-8B/thresholds.json](file://thresholds/Qwen3-8B/thresholds.json)
</cite>

## 更新摘要
**变更内容**
- 新增完整的配置参考文档，提供详细的参数说明和使用指导
- 更新配置参数参考手册，包含运行参数、SAE参数、训练参数、超出指标、钩入点选择、分块、Hadamard旋转、编译保存和日志等完整参数列表
- 增强配置验证规则说明，包含具体的验证逻辑和错误处理
- 完善检查点命名和布局说明，提供详细的文件组织结构
- 更新配置文件格式说明，涵盖顶层配置、SAE配置、分块检查点等格式规范
- 新增 Phase 2 训练框架的完整配置体系，包括四种 SAE 架构变体、JumpReLU 特定参数、Group TopK 参数、Matryoshka 损失配置、正交正则化等

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [依赖关系分析](#依赖关系分析)
7. [性能考量](#性能考量)
8. [故障排除指南](#故障排除指南)
9. [结论](#结论)
10. [附录](#附录)

## 简介
本文件面向 Sparsify 配置管理系统，系统性阐述 TrainConfig 与 SparseCoderConfig 的设计架构、参数体系与运行机制。内容覆盖训练配置选项、SAE 配置参数、设备配置、配置验证机制、默认值与继承规则、配置文件格式与动态更新策略，并提供完整参数参考、最佳实践与常见问题解决方案。

**更新** 新增了完整的配置参考文档，提供了详细的参数说明和使用指导，涵盖所有配置参数的含义、行为说明和使用示例。特别针对 Phase 2 训练框架，新增了四种 SAE 架构变体的选择机制，包括 JumpReLU、Group TopK、Gated SAE 等高级架构的配置参数。

## 项目结构
Sparsify 的配置系统围绕两个核心配置类展开：
- SparseCoderConfig：定义稀疏编码器（SAE）的架构参数
- TrainConfig：定义训练循环、日志、数据集与运行时参数

此外，系统通过 Trainer 统一解析与执行配置，设备抽象层 device.py 提供跨 CUDA/NPU 的统一接口，检查点模块 checkpoint.py 负责保存与加载配置与权重，tiled_sparse_coder.py 支持分块训练模式，utils.py 提供工具函数，__main__.py 提供命令行入口与运行参数。

```mermaid
graph TB
subgraph "配置层"
A["TrainConfig<br/>训练配置"]
B["SparseCoderConfig<br/>SAE架构配置"]
end
subgraph "执行层"
C["Trainer<br/>训练器"]
D["SparseCoder<br/>稀疏编码器"]
E["TiledSparseCoder<br/>分块编码器"]
end
subgraph "基础设施"
F["device.py<br/>设备抽象"]
G["checkpoint.py<br/>检查点"]
H["utils.py<br/>工具函数"]
I["__main__.py<br/>命令行入口"]
end
A --> C
B --> C
C --> D
C --> E
C --> F
C --> G
C --> H
I --> C
```

**图表来源**
- [sparsify/config.py:7-149](file://sparsify/config.py#L7-L149)
- [sparsify/trainer.py:39-760](file://sparsify/trainer.py#L39-L760)
- [sparsify/sparse_coder.py:36-269](file://sparsify/sparse_coder.py#L36-L269)
- [sparsify/tiled_sparse_coder.py:17-342](file://sparsify/tiled_sparse_coder.py#L17-L342)
- [sparsify/device.py:1-118](file://sparsify/device.py#L1-L118)
- [sparsify/checkpoint.py:1-302](file://sparsify/checkpoint.py#L1-L302)
- [sparsify/utils.py:1-227](file://sparsify/utils.py#L1-L227)
- [sparsify/__main__.py:31-211](file://sparsify/__main__.py#L31-L211)

**章节来源**
- [sparsify/config.py:7-149](file://sparsify/config.py#L7-L149)
- [sparsify/__main__.py:31-211](file://sparsify/__main__.py#L31-L211)

## 核心组件
- TrainConfig：集中管理训练生命周期、日志、梯度累积、微批、最大 token 数、学习率、辅助损失、死特征阈值、钩子点选择、初始化种子、层列表与步长、分块训练（tiles）、全局 top-k、输入混合、Hadamard 旋转、模型编译、保存与日志等。
- SparseCoderConfig：集中管理 SAE 架构参数，包括扩展因子、解码器归一化、潜在特征数量、稀疏度 k 等，以及 Phase 2 新增的架构变体参数。
- Trainer：负责根据配置解析钩子点、初始化 SAE、构建优化器、执行前向钩子、计算指标、保存检查点与日志。
- 设备抽象：统一 CUDA/NPU/CPU 的设备检测、bf16 支持、事件计时、分布式后端等。
- 检查点：支持常规与分块 SAE 的保存/加载、肘部阈值加载、Hadamard 状态恢复。
- 分块编码器：在隐藏维上切分为多个独立 SAE，支持全局 top-k 与输入混合。

**章节来源**
- [sparsify/config.py:28-149](file://sparsify/config.py#L28-L149)
- [sparsify/trainer.py:39-760](file://sparsify/trainer.py#L39-L760)
- [sparsify/device.py:1-118](file://sparsify/device.py#L1-L118)
- [sparsify/checkpoint.py:1-302](file://sparsify/checkpoint.py#L1-L302)
- [sparsify/tiled_sparse_coder.py:17-342](file://sparsify/tiled_sparse_coder.py#L17-L342)

## 架构总览
配置系统采用"声明式配置 + 运行时解析"的架构。命令行入口 RunConfig 继承 TrainConfig，统一暴露所有训练参数；Trainer 在启动阶段解析钩子点与层列表，结合设备抽象与工具函数完成初始化；训练过程中根据配置进行梯度累积、微批、日志记录、检查点保存与恢复。

```mermaid
sequenceDiagram
participant CLI as "命令行入口"
participant RC as "RunConfig"
participant TC as "TrainConfig"
participant TR as "Trainer"
participant DEV as "设备抽象"
participant CKPT as "检查点"
CLI->>RC : 解析参数
RC->>TC : 继承训练配置
CLI->>TR : 创建训练器(传入配置)
TR->>DEV : 获取设备类型/设置设备
TR->>TR : 解析钩子点/层列表
TR->>TR : 初始化SAE(普通/分块)
TR->>CKPT : 加载检查点(可选)
TR->>TR : 训练循环(前向钩子/反向/日志/保存)
```

**图表来源**
- [sparsify/__main__.py:31-211](file://sparsify/__main__.py#L31-L211)
- [sparsify/trainer.py:39-760](file://sparsify/trainer.py#L39-L760)
- [sparsify/device.py:1-118](file://sparsify/device.py#L1-L118)
- [sparsify/checkpoint.py:101-302](file://sparsify/checkpoint.py#L101-L302)

## 详细组件分析

### TrainConfig 参数体系与验证规则
- 训练控制
  - 批大小、梯度累积步数、微批步数、最大 token 数、学习率、辅助损失系数、死特征阈值
- 钩子点与层选择
  - 显式钩子点列表、初始化种子列表、层索引列表、层步长
- 分块训练
  - 分块数量、全局 top-k、输入混合
- Hadamard 旋转
  - 启用开关、块大小（必须为正的 2 的幂）、随机种子、是否使用置换
- 模型编译
  - 是否对 Transformer 层进行编译（仅 CUDA 生效）
- 日志与保存
  - 保存频率、保存最佳、保存目录、W&B 开关与项目名、日志频率、微调路径
- **新增** Matryoshka 多 K 训练
  - matryoshka_ks：启用 Multi-K 重构损失时使用的 K 值列表
  - matryoshka_weights：对应 matryoshka_ks 的损失权重
- **新增** 正交性正则化
  - ortho_lambda：解码器正交性正则权重
- **新增** 残差 SAE 训练
  - residual_from：Level 1 SAE checkpoint 路径；设置后训练残差 SAE
- **新增** 优化器选择
  - optimizer：支持 'signum'、'adam'、'muon'

验证规则（运行时检查）：
- 不能同时指定层列表与层步长
- 至少需要一个初始化种子
- 肘部阈值文件存在性校验
- Hadamard 块大小必须为正的 2 的幂
- 非 CUDA 平台自动禁用模型编译
- **新增** 架构验证：必须是 "topk"、"gated"、"jumprelu"、"group_topk" 之一
- **新增** Group TopK 验证：num_groups 和 active_groups 必须 > 0，且 active_groups ≤ num_groups
- **新增** Matryoshka 验证：如果提供 weights，长度必须与 ks 相同

```mermaid
flowchart TD
Start(["进入 __post_init__"]) --> CheckLayers["检查是否同时指定 layers 与 layer_stride"]
CheckLayers --> LayersOK{"条件满足？"}
LayersOK --> |否| Raise1["抛出错误"]
LayersOK --> |是| CheckSeeds["检查 init_seeds 非空"]
CheckSeeds --> SeedsOK{"条件满足？"}
SeedsOK --> |否| Raise2["抛出错误"]
SeedsOK --> CheckAlphas["检查 exceed_alphas 全为正"]
CheckAlphas --> AlphasOK{"条件满足？"}
AlphasOK --> |否| Raise3["抛出错误"]
AlphasOK --> CheckElbow["检查 elbow_threshold_path 存在"]
CheckElbow --> ElbowOK{"条件满足？"}
ElbowOK --> |否| Raise4["抛出错误"]
ElbowOK --> CheckCompile["检查 compile_model 与设备类型"]
CheckCompile --> CompileOK{"条件满足？"}
CompileOK --> |否| DisableCompile["禁用 compile_model"]
CompileOK --> CheckHadamard["检查 hadamard_block_size 为正的2的幂"]
CheckHadamard --> HadamardOK{"条件满足？"}
HadamardOK --> |否| Raise5["抛出错误"]
HadamardOK --> CheckArch["检查 sae.architecture 有效性"]
CheckArch --> ArchOK{"条件满足？"}
ArchOK --> |否| Raise6["抛出错误"]
ArchOK --> CheckGroupTopK["检查 group_topk 参数"]
CheckGroupTopK --> GroupTopKOK{"条件满足？"}
GroupTopKOK --> |否| Raise7["抛出错误"]
GroupTopKOK --> CheckMatryoshka["检查 matryoshka_ks 与 weights"]
CheckMatryoshka --> MatryoshkaOK{"条件满足？"}
MatryoshkaOK --> |否| Raise8["抛出错误"]
MatryoshkaOK --> End(["结束"])
```

**图表来源**
- [sparsify/config.py:165-220](file://sparsify/config.py#L165-L220)

**章节来源**
- [sparsify/config.py:28-149](file://sparsify/config.py#L28-L149)
- [docs/training/config-reference.md:160-170](file://docs/training/config-reference.md#L160-L170)

### SparseCoderConfig 参数体系
- 扩展因子：当潜在特征数为 0 时，潜在维度 = 输入维度 × 扩展因子
- 解码器归一化：是否将解码器权重按行归一到单位范数
- 潜在特征数：显式指定潜在特征数；为 0 时由扩展因子推导
- 稀疏度 k：每样本激活的非零特征数
- **新增** 架构选择：architecture，支持 'topk'、'gated'、'jumprelu'、'group_topk'
- **新增** JumpReLU 参数：jumprelu_init_threshold、jumprelu_bandwidth
- **新增** Group TopK 参数：num_groups、active_groups

分块模式下的行为：
- k 必须能被分块数整除，每个分块的激活预算为 k_per_tile = k / num_tiles

**章节来源**
- [sparsify/config.py:7-26](file://sparsify/config.py#L7-L26)
- [docs/training/config-reference.md:38-54](file://docs/training/config-reference.md#L38-L54)

### SAE 架构变体详解

#### 标准 Top-K SAE
标准的稀疏自编码器，使用固定的 K 值进行稀疏激活选择。

#### Gated SAE
独立的门控分支和幅度分支，门控分支产生连续的选择分数，幅度分支产生系数值，最终输出为两者的逐元素乘积。

#### JumpReLU SAE
具有可学习的每特征阈值的跳跃线性单元，使用直通估计器（STE）进行梯度传播，保持固定 K 的输出形状。

#### Group TopK SAE
引入分组结构的 TopK 选择，首先通过独立路由器选择 g 个组，然后在选定组的潜在特征并集上进行全局 TopK 选择。

**章节来源**
- [sparsify/sparse_coder.py:289-302](file://sparsify/sparse_coder.py#L289-L302)
- [sparsify/gated_sparse_coder.py:12-77](file://sparsify/gated_sparse_coder.py#L12-L77)
- [sparsify/jumprelu_sparse_coder.py:12-69](file://sparsify/jumprelu_sparse_coder.py#L12-L69)
- [sparsify/group_topk_sparse_coder.py:12-84](file://sparsify/group_topk_sparse_coder.py#L12-L84)

### Trainer 生命周期与钩子点解析
- 钩子点解析：支持通配符与范围语法，自动展开为具体模块名
- 层列表解析：若未指定钩子点，则基于模型的层列表生成
- SAE 初始化：根据配置创建普通或分块 SAE，支持多种子
- **新增** Matryoshka 多 K 损失：在标准 FVU 损失基础上，对不同 K 值的子集重构添加加权损失
- **新增** 正交性正则化：对活动解码器列的 Gram 矩阵施加正交性约束
- 训练循环：注册前向钩子捕获输入激活，应用 Hadamard 旋转（可选），计算 FVU/AuxK/Exceed 指标，执行梯度累积与优化器步进，周期性保存检查点

```mermaid
sequenceDiagram
participant TR as "Trainer"
participant MP as "模型"
participant SAE as "SAE/分块SAE"
participant HAD as "Hadamard旋转"
participant MAT as "Matryoshka损失"
participant ORTHO as "正交正则"
participant LOG as "日志/W&B"
TR->>MP : 注册前向钩子(按钩子点)
MP->>TR : 触发钩子(传入输入激活)
TR->>HAD : 可选 : 应用Hadamard变换
TR->>SAE : 编码/解码(含死特征掩码)
SAE-->>TR : 返回重构输出/指标
TR->>MAT : 计算多K子集损失
MAT-->>TR : 返回加权损失
TR->>ORTHO : 计算正交性正则
ORTHO-->>TR : 返回正交损失
TR->>LOG : 计算并记录FVU/AuxK/Exceed
TR->>TR : 梯度累积/优化器步进
TR->>TR : 周期性保存检查点
```

**图表来源**
- [sparsify/trainer.py:39-760](file://sparsify/trainer.py#L39-L760)
- [sparsify/tiled_sparse_coder.py:102-140](file://sparsify/tiled_sparse_coder.py#L102-L140)

**章节来源**
- [sparsify/trainer.py:39-760](file://sparsify/trainer.py#L39-L760)
- [docs/training/config-reference.md:90-106](file://docs/training/config-reference.md#L90-L106)

### 设备抽象与分布式
- 设备检测：自动识别 CUDA/NPU/CPU，提供统一的 bf16 支持检测与事件计时
- 分布式后端：根据平台选择 nccl/hccl/gloo
- 训练器在 DDP 模式下进行梯度同步与广播，确保日志一致性

**章节来源**
- [sparsify/device.py:1-118](file://sparsify/device.py#L1-L118)
- [sparsify/trainer.py:168-227](file://sparsify/trainer.py#L168-L227)

### 检查点与动态配置更新
- 检查点布局：顶层 config.json、状态文件、优化器状态、各钩子点的 SAE 权重与配置、可选的 Hadamard 状态
- 分块检查点：保存 num_tiles/k_per_tile/global_topk/input_mixing 等元信息，逐 tile 保存权重
- 动态更新：Trainer 在 fit() 中根据配置决定是否启用编译、Hadamard、日志等；检查点加载时恢复配置与权重

```mermaid
flowchart TD
LoadCfg["加载顶层config.json"] --> CheckTiles{"是否分块检查点？"}
CheckTiles --> |是| LoadTiles["逐tile加载SAE权重"]
CheckTiles --> |否| LoadSingle["加载单SAE权重"]
LoadTiles --> LoadMeta["加载优化器/状态/Hadamard"]
LoadSingle --> LoadMeta
LoadMeta --> Resume["恢复训练状态"]
```

**图表来源**
- [sparsify/checkpoint.py:101-302](file://sparsify/checkpoint.py#L101-L302)
- [sparsify/tiled_sparse_coder.py:278-342](file://sparsify/tiled_sparse_coder.py#L278-L342)

**章节来源**
- [sparsify/checkpoint.py:101-302](file://sparsify/checkpoint.py#L101-L302)
- [docs/training/config-reference.md:171-193](file://docs/training/config-reference.md#L171-L193)

### 分块稀疏编码器（TiledSparseCoder）
- 输入沿隐藏维切分为 T 个块，每个块独立训练 SAE
- 全局 top-k：所有块竞争相同的 k 激活预算，解码时使用块对角解码器矩阵
- 输入混合：学习 T×T 混合矩阵，在块间传递信息，随后在原始空间重新计算 FVU
- 断言约束：d_in 与 k 必须能被 num_tiles 整除

**章节来源**
- [sparsify/tiled_sparse_coder.py:17-342](file://sparsify/tiled_sparse_coder.py#L17-L342)
- [docs/training/config-reference.md:107-122](file://docs/training/config-reference.md#L107-L122)

## 依赖关系分析
- TrainConfig 依赖 SparseCoderConfig（组合关系）
- Trainer 依赖 TrainConfig、设备抽象、检查点、工具函数、SAE/分块 SAE
- 分块 SAE 依赖普通 SAE 与设备抽象
- 命令行入口 RunConfig 继承 TrainConfig，提供运行参数

```mermaid
graph TB
TrainConfig --> SparseCoderConfig
Trainer --> TrainConfig
Trainer --> SparseCoder
Trainer --> TiledSparseCoder
Trainer --> device_py["device.py"]
Trainer --> checkpoint_py["checkpoint.py"]
Trainer --> utils_py["utils.py"]
RunConfig --> TrainConfig
```

**图表来源**
- [sparsify/config.py:28-149](file://sparsify/config.py#L28-L149)
- [sparsify/trainer.py:39-760](file://sparsify/trainer.py#L39-L760)
- [sparsify/sparse_coder.py:36-269](file://sparsify/sparse_coder.py#L36-L269)
- [sparsify/tiled_sparse_coder.py:17-342](file://sparsify/tiled_sparse_coder.py#L17-L342)
- [sparsify/device.py:1-118](file://sparsify/device.py#L1-L118)
- [sparsify/checkpoint.py:1-302](file://sparsify/checkpoint.py#L1-L302)
- [sparsify/utils.py:1-227](file://sparsify/utils.py#L1-L227)
- [sparsify/__main__.py:31-211](file://sparsify/__main__.py#L31-L211)

**章节来源**
- [sparsify/config.py:28-149](file://sparsify/config.py#L28-L149)
- [sparsify/trainer.py:39-760](file://sparsify/trainer.py#L39-L760)

## 性能考量
- 模型编译：在 CUDA 上对 Transformer 层进行编译可减少小算子内核开销，提升吞吐
- bf16 自动混合精度：在支持的设备上启用可显著提升性能
- 分块训练：通过 num_tiles 将输入宽度切分，提高并行度；global_topk 减少循环解码次数
- 输入混合：在分块之间引入学习的混合矩阵，可能带来额外计算，但有助于跨块信息流动
- 梯度累积与微批：通过 grad_acc_steps 与 micro_acc_steps 控制有效损失归一化与内存占用
- 日志与计时：在 CUDA/NPU 上使用事件计时，避免 Python 层计时开销
- **新增** Matryoshka 多 K 训练：通过联合优化多个 K 值的重构质量，可能降低对高 K 值的需求
- **新增** 正交性正则化：鼓励解码器基向量正交，提高特征独立性，可能减少所需的激活数

## 故障排除指南
- 配置验证失败
  - 同时指定 layers 与 layer_stride：修改其一
  - 未提供 init_seeds：至少提供一个种子
  - exceed_alphas 包含非正值：修正为正数
  - elbow_threshold_path 文件不存在：确认路径与文件存在
  - hadamard_block_size 不是正的 2 的幂：调整为合法值
  - 非 CUDA 平台 compile_model：将被自动禁用，如需请切换到 CUDA
  - **新增** sae.architecture 不在有效集合中：使用 'topk'、'gated'、'jumprelu'、'group_topk' 之一
  - **新增** Group TopK 参数无效：确保 num_groups > 0、active_groups > 0 且 active_groups ≤ num_groups
  - **新增** Matryoshka 权重长度不匹配：确保 matryoshka_weights 与 matryoshka_ks 长度相同
- 分块训练断言失败
  - d_in 或 k 无法被 num_tiles 整除：调整 d_in/k 或 num_tiles
- 检查点加载异常
  - 分块与非分块配置不匹配：确保 num_tiles 一致
  - 分块数量不一致：调整当前配置或检查点
- 日志与 W&B
  - W&B 初始化失败：自动降级为关闭日志，检查网络与环境变量

**章节来源**
- [sparsify/config.py:124-149](file://sparsify/config.py#L124-L149)
- [sparsify/checkpoint.py:44-73](file://sparsify/checkpoint.py#L44-L73)
- [sparsify/tiled_sparse_coder.py:38-43](file://sparsify/tiled_sparse_coder.py#L38-L43)
- [sparsify/trainer.py:186-227](file://sparsify/trainer.py#L186-L227)

## 结论
Sparsify 的配置系统以 TrainConfig 与 SparseCoderConfig 为核心，通过 Trainer 实现统一解析与执行，辅以设备抽象与检查点模块，形成可扩展、可移植且高性能的训练配置框架。系统提供了严格的参数验证、灵活的分块训练与混合策略、完善的日志与检查点机制，适用于多平台与大规模训练场景。

**更新** 新增的 Phase 2 训练框架进一步增强了配置系统的灵活性，支持四种不同的 SAE 架构变体，包括 JumpReLU、Group TopK、Gated SAE 等高级架构，以及 Matryoshka 多 K 训练和正交性正则化等训练增强技术，为不同应用场景提供了更多优化选项。

## 附录

### 配置参数参考手册
**更新** 基于新的配置参考文档，提供完整的参数说明和使用指导

- 运行参数（RunConfig）
  - 模型名称/路径：Hugging Face 模型名称或本地路径
  - 数据集名称/路径：Hugging Face 数据集、本地数据集路径或 .bin memmap 数据集
  - 数据集分割：要加载的数据集分割
  - 上下文长度：分词或 memmap 读取时使用的上下文长度
  - Hugging Face 令牌：用于门控模型的 Hugging Face 令牌
  - 模型版本：可选的模型版本
  - 最大样本数：可选的数据集上限
  - 断点续训：从现有检查点树恢复
  - 文本列名：未分词数据集的文本列名
  - 数据洗牌种子：数据集随机种子
  - 预处理进程数：分词工作进程数
  - 额外数据集参数：编码为 k=v,k=v 的额外 load_dataset() 参数

- SAE 参数（SparseCoderConfig）
  - 扩展因子：当 num_latents == 0 时潜在维度的扩展因子
  - 解码器归一化：将解码器行归一化为单位范数
  - 潜在变量计数：显式潜在变量计数；0 表示从扩展因子派生
  - 稀疏度 k：每个样本的活跃潜在变量数
  - **新增** 架构：'topk' | 'gated' | 'jumprelu' | 'group_topk'
  - **新增** JumpReLU 参数：阈值初始化、平滑带宽
  - **新增** Group TopK 参数：分组数、激活分组数

- 训练参数（TrainConfig）
  - 批大小：序列批次大小
  - 梯度累积步数：外部训练步骤中的梯度累积
  - 微批次步数：微批次拆分因子
  - 最大令牌数：可选的总令牌预算
  - 基础学习率：基础学习率；None 使用 Trainer.__init__() 中的缩放规则
  - AuxK 损失权重：AuxK 死特征损失的权重
  - 死特征阈值：特征被认为死亡前自上次激活以来的令牌数
  - **新增** Matryoshka 多 K 训练：K 值列表、权重
  - **新增** 正交性正则化：正交性约束权重
  - **新增** 残差训练：Level 1 SAE 路径
  - **新增** 优化器选择：'signum' | 'adam' | 'muon'

- 超出指标参数
  - 超出系数：计算超出比率时应用于肘部值的乘数
  - 肘部阈值路径：包含预计算肘部值的 JSON 文件

- 钩入点选择参数
  - 钩入点列表：模块名称或模式的显式列表
  - 初始化种子：用于每个钩入点初始化一个或多个 SAE 的随机种子
  - 层索引：未提供钩入点时使用的层索引
  - 层步长：在已解析钩入点上的子采样步幅

- 分块参数
  - 分块数量：将激活宽度拆分为独立的分块
  - 全局 top-k：在所有分块预激活上竞争一个全局 top-k
  - 输入混合：在分块级编码前学习一个分块空间混合矩阵

- Hadamard 旋转参数
  - 启用 Hadamard：启用块对角 Hadamard 旋转
  - Hadamard 块大小：块大小；必须是正的 2 的幂
  - Hadamard 种子：置换的种子
  - 使用置换：是否在变换前应用置换

- 编译、保存和日志参数
  - 编译模型：使用 torch.compile 编译 Transformer 层；仅在 CUDA 上激活
  - 保存频率：每 N 个优化器步骤保存
  - 保存最佳：保存每个钩入点的最佳检查点
  - 保存目录：基础输出目录
  - 日志到 W&B：启用 Weights & Biases 日志
  - 运行名称：可选的运行名称前缀
  - W&B 项目：可选的 W&B 项目覆盖
  - W&B 日志频率：日志频率
  - 微调路径：用于初始化 SAE 的检查点树路径

**章节来源**
- [docs/training/config-reference.md:12-193](file://docs/training/config-reference.md#L12-L193)

### 配置文件格式与动态更新
**更新** 基于新的配置参考文档，提供详细的配置文件格式说明

- 配置文件格式
  - 顶层：config.json（训练配置）
  - SAE：cfg.json（SAE 配置）+ sae.safetensors（权重）
  - 分块：顶层 cfg.json（含 num_tiles/k_per_tile/global_topk/input_mixing）+ tile_i/cfg.json 与 tile_i/sae.safetensors
  - 可选：hadamard_rotations.pt（Hadamard 状态）

- 动态配置更新
  - 训练器在 fit() 中根据配置决定是否启用编译、Hadamard、日志等
  - 检查点加载时恢复配置与权重，支持断点续训与微调

- 检查点命名和布局
  - 新运行构建如下名称：`<run_name>_dp<world_size>_bs<batch_size>_ga<grad_acc_steps>_ef<expansion_factor>_k<k>_<timestamp>`
  - 保存运行中的重要文件：
    - config.json：序列化的训练配置
    - state.pt：global_step 和 total_tokens
    - optimizer_0.pt：优化器状态
    - rank_0_state.pt：死特征计数器和最佳损失元数据
    - `<hookpoint>/cfg.json`：每个 SAE 的配置
    - `<hookpoint>/sae.safetensors`：SAE 权重
  - 分块检查点存储：
    - 顶层 cfg.json，包含 num_tiles、k_per_tile、global_topk 和 input_mixing
    - 每个分块的 tile_<i>/sae.safetensors
    - 启用输入混合时的可选 mixing.pt

**章节来源**
- [docs/training/config-reference.md:171-193](file://docs/training/config-reference.md#L171-L193)
- [sparsify/checkpoint.py:199-302](file://sparsify/checkpoint.py#L199-L302)

### 使用示例与最佳实践
**更新** 基于新的配置参考文档，提供使用示例和最佳实践

- 示例：命令行训练
  - 使用 RunConfig 指定模型、数据集、上下文长度、断点续训等参数
  - 通过 hookpoints/layers 选择训练模块，设置 init_seeds 进行多种子实验

- 最佳实践
  - 合理设置 batch_size 与 grad_acc_steps 以平衡显存与吞吐
  - 在 CUDA 上启用 compile_model 以减少小算子开销
  - 使用分块训练提升并行度，确保 d_in 与 k 能被 num_tiles 整除
  - 启用 W&B 记录指标，定期保存检查点
  - 使用 elbow 阈值文件与 exceed 指标评估重建质量
  - **新增** 选择合适的 SAE 架构：根据任务需求选择标准 TopK、Gated、JumpReLU 或 Group TopK
  - **新增** 配置 Matryoshka 训练：为不同 K 值设置权重，平衡重构质量和计算开销
  - **新增** 应用正交性正则化：在需要高特征独立性的场景下启用 ortho_lambda

- 配置验证规则
  - layers 和 layer_stride 不能同时指定
  - init_seeds 不能为空
  - exceed_alphas 必须为正
  - elbow_threshold_path 在提供时必须存在
  - hadamard_block_size 必须是正的 2 的幂
  - compile_model 在非 CUDA 后端上被静默禁用
  - **新增** sae.architecture 必须在有效集合中
  - **新增** Group TopK 参数必须满足约束条件
  - **新增** Matryoshka 权重长度必须与 K 值列表匹配

**章节来源**
- [sparsify/__main__.py:31-211](file://sparsify/__main__.py#L31-L211)
- [docs/training/config-reference.md:12-193](file://docs/training/config-reference.md#L12-L193)

### 肘部阈值与超参调优
**更新** 基于新的配置参考文档，提供肘部阈值和超参数调优指导

- 肘部阈值文件格式
  - 键为钩子点名称，值包含 elbow_p 与 elbow_value
  - 支持层/组件启发式匹配，自动从层号和组件名推断对应的阈值

- 超参与评估
  - 通过 exceed_alphas 与 elbow_threshold_path 计算超过阈值的比例，作为重建误差的评估指标
  - 超出指标仅为可以匹配到肘部值的钩入点计算
  - 训练期间，超出比率基于绝对重建误差计算，并记录为 exceed_alpha_<alpha>

- 阈值加载策略
  - 直接匹配：精确匹配钩入点名称
  - 层组件匹配：提取层号和组件名进行启发式匹配
  - 搜索模式：支持 layer_{num}/{component} 和 layer_{num}/{component_underscore} 等模式

**章节来源**
- [thresholds/Qwen3-8B/thresholds.json:1-130](file://thresholds/Qwen3-8B/thresholds.json#L1-L130)
- [sparsify/checkpoint.py:104-147](file://sparsify/checkpoint.py#L104-L147)
- [docs/training/config-reference.md:77-89](file://docs/training/config-reference.md#L77-L89)

### Phase 2 训练框架特性详解

#### SAE 架构变体选择
- **Top-K SAE**：标准稀疏编码器，固定 K 值激活
- **Gated SAE**：分离门控和幅度分支，提高特征利用效率
- **JumpReLU SAE**：可学习阈值的跳跃线性单元，保持固定 K 输出
- **Group TopK SAE**：引入分组结构，先路由后选择

#### Matryoshka 多 K 训练
- 在同一次训练中同时优化多个 K 值的重构质量
- 通过加权组合不同 K 值的损失，强制学习特征重要性排序
- 支持优雅降级：部署时可按较小 K 值截断

#### 正交性正则化
- 鼓励解码器基向量正交，提高特征独立性
- 通过 Gram 矩阵与单位矩阵的差异施加约束
- 改善共轭梯度收敛速度，减少冗余特征

**章节来源**
- [sparsify/config.py:23-41](file://sparsify/config.py#L23-L41)
- [sparsify/trainer.py:520-551](file://sparsify/trainer.py#L520-L551)
- [sparsify/jumprelu_sparse_coder.py:12-69](file://sparsify/jumprelu_sparse_coder.py#L12-L69)
- [sparsify/group_topk_sparse_coder.py:12-84](file://sparsify/group_topk_sparse_coder.py#L12-L84)
- [sparsify/gated_sparse_coder.py:12-77](file://sparsify/gated_sparse_coder.py#L12-L77)