...
- [scripts/scriptA100_legacy.sh](file://scripts/scriptA100_legacy.sh)
</cite>

## 更新摘要
**变更内容**
- 新增 Qwen3-0.6B 训练脚本和完整训练流程
- 更新脚本归档结构，所有训练脚本迁移至 `scripts/archive/first_time_train/` 目录
- 新增阈值计算工具和完整的阈值文件管理
- 更新开发环境设置和脚本使用指南
...
**更新** 项目结构已更新，新增 Qwen3-0.6B 训练脚本和完整的脚本归档结构。
...
D --> D1["archive 归档脚本<br/>first_time_train/Qwen3-*/"]
D --> D2["train_qwen3_0p6b.sh<br/>Qwen3-0.6B 专用训练脚本"]
...
A --> J["thresholds 阈值文件<br/>Qwen3-0.6B/0.4B/8B"]
A --> K["compute_elbow_thresholds.py<br/>阈值计算工具"]
```
...
### Qwen3 模型训练脚本
**新增** 项目现在包含针对不同 Qwen3 模型大小的完整训练脚本，支持从 0.6B 到 8B 参数量的模型。

#### Qwen3-0.6B 训练脚本
- 专用训练脚本：`scripts/train_qwen3_0p6b.sh` 提供完整的 Qwen3-0.6B 训练配置
- 参数配置：支持分布式训练、阈值文件路径、W&B 项目配置等
- 性能优化：包含 NVTX 分析和模型编译选项

#### 首次训练脚本归档
- 结构迁移：所有 Qwen3 模型的首次训练脚本已迁移至 `scripts/archive/first_time_train/` 目录
- Qwen3-0.6B：包含完整的 q、o、up 三个投影家族训练流程
- Qwen3-4B：包含完整的 q、o、up 三个投影家族训练流程  
- Qwen3-8B：包含完整的 q、o、up 三个投影家族训练流程

#### 阈值计算与管理
- 阈值计算工具：`compute_elbow_thresholds.py` 提供自动化的拐点阈值计算
- 阈值文件：为每个 Qwen3 模型提供完整的阈值文件（q、o、up 三类）
- 自动化流程：脚本自动处理激活收集、阈值计算和结果保存
...
### 传统脚本与兼容性
- 传统脚本：`scripts/scriptA100_legacy.sh` 提供 A100 GPU 的传统训练配置
- 兼容性：保留对旧版训练流程的支持，便于迁移和兼容性测试

**章节来源**
- [scripts/scriptA100_legacy.sh:1-62](file://scripts/scriptA100_legacy.sh#L1-L62)
...
- Qwen3 模型训练问题：检查阈值文件路径正确性，确认模型权重可用性。
...
## 结论
本指南系统梳理了 Sparsify 的开发环境、依赖与构建、代码与提交规范、审查流程、架构决策与技术选型、扩展机制、贡献与问题处理、历史文档与演进、调试与性能优化等关键内容。**更新** 项目现已支持完整的 Qwen3 模型训练流程，包含从 0.6B 到 8B 参数量的完整训练脚本和阈值管理系统。建议新贡献者先阅读概览与训练快速入门，再深入配置参考与训练流水线，最后结合导出与脚本工具开展实验与生产化流程。
...
- **新增** Qwen3 模型支持：确保安装必要的 Transformers 和 Datasets 依赖以支持 Qwen3 模型加载。
...
- **新增** Qwen3 模型支持：扩展了对大规模语言模型的训练支持，包含完整的阈值计算和训练脚本。
...
- **新增** Qwen3 模型扩展：支持不同参数量的 Qwen3 模型训练，包含完整的阈值计算和训练流程。
...
- **新增** 脚本归档结构：训练脚本已迁移至 archive 目录，便于历史参考和兼容性维护。
...
### Qwen3 模型训练指南
**新增** 详细的 Qwen3 模型训练使用指南：

#### Qwen3-0.6B 训练流程
1. **阈值计算**：使用 `compute_elbow_thresholds.py` 为 Qwen3-0.6B 模型计算激活阈值
2. **训练脚本**：使用 `scripts/train_qwen3_0p6b.sh` 进行完整的训练
3. **参数配置**：支持分布式训练、阈值文件路径、W&B 项目配置等

#### 首次训练脚本使用
- **Qwen3-0.6B**：包含 q、o、up 三个投影家族的完整训练流程
- **Qwen3-4B**：包含 q、o、up 三个投影家族的完整训练流程  
- **Qwen3-8B**：包含 q、o、up 三个投影家族的完整训练流程

#### 阈值文件管理
- **文件位置**：位于 `thresholds/Qwen3-*/` 目录下
- **文件类型**：包含 thresholds_q.json、thresholds_o.json、thresholds_up.json
- **使用方法**：在训练脚本中通过 `--elbow_threshold_path` 参数指定
