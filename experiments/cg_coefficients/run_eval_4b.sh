#!/bin/bash
# CG Coefficients Phase 1 — Qwen3-4B 全层实验
#
# 用法:
#   bash experiments/cg_coefficients/run_eval_4b.sh
#
# 可选环境变量:
#   LAYERS="0 5 10 15 20 25 30 35"  # 自定义层范围 (默认采样 8 层)
#   NUM_SAMPLES=4096                 # 样本数 (默认 4096)
#   CG_MAX_ITER=10                   # CG 迭代数 (默认 10)
#
# 注意: Qwen3-4B 的 LUT 只有 gate_up_proj 和 qkv_proj，没有 o_proj

set -euo pipefail

MODEL=/root/models/Qwen3-4B
LUT_DIR=/root/models/Qwen3-4B/lut
THRESHOLD_DIR=/root/sparsify-ascend/thresholds/Qwen3-4B
DATASET=/root/fineweb-edu/sample/10BT-tokenized-qwen3-2048
RESULT_DIR=experiments/cg_coefficients/results/Qwen3-4B
