...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate baseline vs expert-only W8A8 simulation for product_key_expert_jumprelu checkpoints.",
    )
...
def prepare_quantized_state(sae: ProductKeyExpertJumpReLUSparseCoder) -> QuantizedExpertState:
...
@torch.no_grad()
def forward_w8a8_expert_only(
    sae: ProductKeyExpertJumpReLUSparseCoder,
    quant_state: QuantizedExpertState,
    x: Tensor,
) -> Tensor:
    ...
    expert_logits = sae._expert_logits_from_flat(flat_x)
    selected_expert_idx, selected_probs = sae._select_expert_route(expert_logits)

    selected_weight = quant_state.q_weight[selected_expert_idx.long()]
    selected_scales = quant_state.weight_scales[selected_expert_idx.long()]
...
def main() -> None:
    ...
    quant_states: dict[str, QuantizedExpertState] = {}
    for hookpoint in hookpoints:
        ...
        saes[hookpoint] = sae
        quant_states[hookpoint] = prepare_quantized_state(sae)
    ...
    for batch in dataloader:
        ...
        for hookpoint in hookpoints:
            ...
            quant_recon = forward_w8a8_expert_only(sae, quant_states[hookpoint], acts)
