...
from quantization.quant_utils import simulate_w8a8_sparse_decode_prequantized
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate baseline vs full-encoder W8A8 plus decoder-W8A8 simulation for product_key_expert_jumprelu checkpoints.",
    )
...
def decode_w8a8(
    ...
    sparse_out = simulate_w8a8_sparse_decode_prequantized(
        flat_indices,
        flat_acts,
...
@torch.no_grad()
def forward_w8a8_full_encoder_w8a8_decoder(
    sae: ProductKeyExpertJumpReLUSparseCoder,
    quant_state: QuantizedFullStackState,
    x: Tensor,
) -> Tensor:
    top_acts, top_indices = encode_w8a8_full_encoder(sae, quant_state.encoder, x)
    sparse_out = decode_w8a8(sae, quant_state.decoder, top_acts, top_indices)
...
def main() -> None:
    ...
    for batch in dataloader:
        ...
        for hookpoint in hookpoints:
            ...
            quant_recon = forward_w8a8_full_encoder_w8a8_decoder(
                sae,
        ...
        json.dump(
            {
                ...
                "device": device_str,
                "alpha": args.alpha,
                "quantization_scope": "full_encoder_w8a8_decoder",
