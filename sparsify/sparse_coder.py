...
class SparseCoder(nn.Module):
    ...
    def _set_monitoring_metrics(
        self, metrics: dict[str, Tensor | float | int]
    ) -> None:
        ...
        for key, value in metrics.items():
            if isinstance(value, Tensor):
                if value.numel() != 1:
                    raise ValueError(
                        f"Monitoring metric '{key}' must be scalar, got shape={tuple(value.shape)}"
                    )
                payload[key] = float(value.detach())
            else:
                payload[key] = float(value)
        self._last_monitoring_metrics = payload

    def pop_monitoring_metrics(self) -> dict[str, float]:
        metrics = getattr(self, "_last_monitoring_metrics", {})
        self._last_monitoring_metrics = {}
        return metrics

    def dead_metric_slices(self) -> dict[str, slice]:
        return {}

    def set_training_tokens(self, total_tokens: int) -> None:
        self._training_tokens = int(total_tokens)
    ...
    def pop_regularization_loss(self) -> Tensor | None:
        loss = getattr(self, "_last_regularization_loss", None)
        self._last_regularization_loss = None
        return loss
...
def _get_sae_class(architecture: str) -> type:
    ...
    if architecture == "shared_stacked_product_key_expert_jumprelu":
        return SharedStackedProductKeyExpertJumpReLUSparseCoder
    if architecture == "shared_lowrank_product_key_expert_jumprelu":
        return SharedLowRankProductKeyExpertJumpReLUSparseCoder
    if architecture == "shared_lowrank_cascade_product_key_expert_jumprelu":
        return SharedLowRankCascadeProductKeyExpertJumpReLUSparseCoder
    ...
    if architecture == "residual_dual_shared_product_key_expert_residual":
        return ResidualDualSharedProductKeyExpertResidualSparseCoder
    ...
    if architecture == "lowrank_product_key_expert_jumprelu":
        return LowRankProductKeyExpertJumpReLUSparseCoder
...
def _select_active_expert_indices_from_probs(
    router_probs: Tensor,
    active_experts: int,
) -> tuple[Tensor, Tensor]:
...
def _select_adaptive_active_expert_indices_from_probs(
    router_probs: Tensor,
    active_experts: int,
    *,
...
def _select_adaptive_active_expert_indices(
    ...
    *,
    ...
    return _select_adaptive_active_expert_indices_from_probs(
        router_probs,
        active_experts,
        easy_share_threshold=easy_share_threshold,
    )
...
class SharedTwoStageResidualExpertSparseCoder(SparseCoder):
    ...
    def forward(
        ...
        if num_dead > 0:
            ...
            e_hat = self._decode_sparse(auxk_acts, auxk_indices)
            auxk_loss = (e_hat - e.detach()).pow(2).sum()
            auxk_loss = scale * auxk_loss / total_variance
        else:
            auxk_loss = sae_out.new_tensor(0.0)
...
class SharedProductKeyExpertJumpReLUSparseCoder(SparseCoder):
    ...
    def _router_temperature(self) -> float:
        warmup_tokens = self.cfg.router_warmup_tokens
        if warmup_tokens <= 0:
            return 1.0
        start_temp = float(self.cfg.router_warmup_init_temperature)
        progress = min(
            max(self.current_training_tokens(), 0) / warmup_tokens,
            1.0,
        )
        return start_temp + (1.0 - start_temp) * progress
    ...
    def _router_load_balance_loss(self, router_probs: Tensor) -> Tensor:
        target_prob = 1.0 / self.num_experts
        centered = router_probs.mean(dim=0) - target_prob
        return self.num_experts * centered.pow(2).mean()
    ...
    def _revival_probe_candidates(
        ...
        routed_dead_mask = dead_mask[
            self.shared_num_latents : self.shared_num_latents + self.expert_num_latents
        ]
        if not bool(routed_dead_mask.any()):
            return None

        dead_expert_mask = routed_dead_mask.view(
            self.num_experts, self.latents_per_expert
        ).any(dim=-1)
    ...
    def dead_metric_slices(self) -> dict[str, slice]:
        return {
            "shared_dead_pct": slice(0, self.shared_num_latents),
            "routed_dead_pct": slice(
                self.shared_num_latents,
                self.shared_num_latents + self.expert_num_latents,
            ),
        }

    def _record_route_monitoring(
        self,
        expert_logits: Tensor,
        selected_expert_idx: Tensor,
        selected_probs: Tensor,
        *,
        ...
        detached_router_probs = router_probs.detach()
        eps = torch.finfo(router_probs.dtype).eps
        entropy = -(
            detached_router_probs * detached_router_probs.clamp_min(eps).log()
        ).sum(dim=-1).mean()
        top1_share = detached_router_probs.max(dim=-1).values.mean()

        selected_expert_mask = torch.zeros(
            self.num_experts,
            device=selected_expert_idx.device,
            dtype=torch.bool,
        )
        positive_slots = (selected_probs > 0).nonzero(as_tuple=False)
        if positive_slots.numel() > 0:
            positive_expert_idx = selected_expert_idx[
                positive_slots[:, 0], positive_slots[:, 1]
            ].long()
            selected_expert_mask[positive_expert_idx] = True
        expert_selected_pct = selected_expert_mask.float().mean()
        if load_balance_loss is None:
            load_balance_loss = self._router_load_balance_loss(router_probs)

        self._set_monitoring_metrics(
            {
                "expert_selected_pct": expert_selected_pct,
                "router_entropy": entropy,
                "top1_expert_share": top1_share,
                "router_temperature": self._router_temperature(),
                "router_load_balance_loss": load_balance_loss,
            }
        )
    ...
    def forward(
        ...
        expert_logits = self._expert_logits_from_flat(flat_residual)
        router_probs = torch.softmax(expert_logits, dim=-1)
        router_load_balance_loss = self._router_load_balance_loss(router_probs)
        self._set_regularization_loss(router_load_balance_loss)
        selected_expert_idx, selected_probs = self._select_expert_route(
            expert_logits,
            router_probs=router_probs,
        )
        self._record_route_monitoring(
            expert_logits,
            selected_expert_idx,
            selected_probs,
            router_probs=router_probs,
            load_balance_loss=router_load_balance_loss,
        )
        stage2_candidate_acts = self._expert_candidate_acts(
            flat_residual,
            selected_expert_idx,
            selected_probs,
        )
...
class SharedCascadeProductKeyExpertJumpReLUSparseCoder(
    ...
    def _select_ordered_expert_pair_from_logits(
        self,
        expert_logits: Tensor,
        *,
        router_probs: Tensor | None = None,
    ...
    def _select_ordered_expert_pair(
        ...
        flat_x = residual.reshape(-1, self.d_in)
        expert_logits = self._expert_logits_from_flat(flat_x)
        return self._select_ordered_expert_pair_from_logits(expert_logits)
    ...
    def _forward_cascade_state(
        ...
        x_centered: Tensor,
        *,
        need_aux_candidates: bool = False,
        ...
        router_load_balance_loss = self._router_load_balance_loss(router_probs)
        self._set_regularization_loss(router_load_balance_loss)
        selected_expert_idx, selected_probs = (
            self._select_ordered_expert_pair_from_logits(
                expert_logits,
                router_probs=router_probs,
            )
        )
...
class SharedConditionalCascadeProductKeyExpertJumpReLUSparseCoder(
    ...
    def _forward_cascade_state(
        ...
        *,
        ...
        residual_1 = x_centered - stage1_out
        flat_residual_1 = residual_1.reshape(-1, self.d_in)
        expert_logits = self._expert_logits_from_flat(flat_residual_1)
        router_probs = torch.softmax(expert_logits, dim=-1)
        router_load_balance_loss = self._router_load_balance_loss(router_probs)
        self._set_regularization_loss(router_load_balance_loss)
        selected_expert_idx, selected_probs = (
...
class SharedProductKeyExpertResidualSparseCoder(
    ...
    """Shared coarse stage, PK-routed expert cleanup, then tiny global residual cleanup."""

    architecture_name = "shared_product_key_expert_residual"
    ...
    def __init__(
        ...
        *,
        ...
        if self.stage2_k < 2:
            raise ValueError(
                f"{self.architecture_name} requires stage2_k >= 2 so the routed "
                f"expert branch keeps at least one cleanup slot after reserving a "
                f"global residual slot; got stage2_k={self.stage2_k}"
            )

        self.residual_stage_k = max(1, self.stage2_k // 6)
        self.expert_stage_k = self.stage2_k - self.residual_stage_k
        if self.expert_stage_k < 1:
            raise ValueError(
                f"{self.architecture_name} requires expert_stage_k >= 1, "
                f"got stage2_k={self.stage2_k}"
            )

        self.residual_num_latents = self.latents_per_expert
        hidden_dim = min(
            self.latents_per_expert,
            cfg.factorized_hidden_dim or self.latents_per_expert,
        )
        self.residual_factor = nn.Linear(
            d_in, hidden_dim, device=device, dtype=dtype
        )
        self.residual_heads = nn.Parameter(
            torch.empty(
        ...
        self.residual_head_bias = nn.Parameter(
            torch.zeros(
                self.residual_num_latents,
                device=device,
                dtype=dtype,
        ...
        self.residual_factor.bias.data.zero_()
        nn.init.kaiming_uniform_(self.residual_factor.weight, a=5**0.5)
        nn.init.kaiming_uniform_(self.residual_heads, a=5**0.5)

        if decoder:
            assert self.W_dec is not None, "Decoder weight was not initialized."
            residual_decoder = torch.matmul(
                self.residual_heads.data,
                self.residual_factor.weight.data,
            )
            decoder_init = torch.cat((self.W_dec.data, residual_decoder), dim=0)
            self.W_dec = nn.Parameter(decoder_init.clone())
            if self.cfg.normalize_decoder:
                self.set_decoder_norm_to_unit_norm()
    ...
    def _encoder_linear_layers(self):
        return super()._encoder_linear_layers() + [
            ("residual_factor", self.residual_factor)
        ]

    def _extra_encode_accesses(self) -> list[tuple[str, int, str]]:
        base = super()._extra_encode_accesses()
        hidden_dim = self.residual_factor.out_features
        base.append(
            (
                "factorized_residual_head",
                hidden_dim * self.residual_num_latents,
                f"residual={hidden_dim}x{self.residual_num_latents}",
            )
        )
        return base

    def _encode_expert_stage_with_budget(
        self, residual: Tensor, *, topk: int
    ...
    def _encode_residual_stage(
        ...
        return (
            top_acts.reshape(target_shape),
            top_indices.reshape(target_shape),
            residual_acts.reshape(acts_shape),
        )
    ...
    def encode(self, x: Tensor) -> EncoderOutput:
        ...
        residual_1 = x_centered - stage1_out
        stage2_acts, stage2_indices, stage2_full = self._encode_expert_stage_with_budget(
            residual_1,
            topk=self.expert_stage_k,
        )
        stage2_out = self._decode_sparse(stage2_acts, stage2_indices)

        residual_2 = residual_1 - stage2_out
        stage3_acts, stage3_indices, stage3_full = self._encode_residual_stage(
            residual_2
        )

        combined_acts = torch.cat((stage1_acts, stage2_acts, stage3_acts), dim=-1)
        combined_indices = torch.cat(
            (stage1_indices, stage2_indices, stage3_indices), dim=-1
        )
        combined_full = torch.cat((stage1_full, stage2_full, stage3_full), dim=-1)
        return EncoderOutput(combined_acts, combined_indices, combined_full)
...
class SharedAdaptiveActiveProductKeyExpertResidualSparseCoder(
    SharedProductKeyExpertResidualSparseCoder
):
    """Shared PK residual scaffold with adaptive 1-or-2 expert cleanup."""

    architecture_name = "shared_adaptive_active_product_key_expert_residual"
...
class SharedStackedProductKeyExpertJumpReLUSparseCoder(
    SharedProductKeyExpertJumpReLUSparseCoder
):
    """Shared coarse branch plus two residual PK cleanup passes on one expert set."""

    architecture_name = "shared_stacked_product_key_expert_jumprelu"
    ...
    def __init__(
        ...
        *,
        ...
        if self.stage2_k < 2:
            raise ValueError(
                f"{self.architecture_name} requires "
                "at least two routed cleanup latents so the residual pass is non-empty; "
                f"got stage2_k={self.stage2_k}"
            )
        self.stage2a_k = max(1, self.stage2_k // 2)
        self.stage2b_k = self.stage2_k - self.stage2a_k
    ...
    def _select_expert_set(self, residual: Tensor) -> tuple[Tensor, Tensor]:
        flat_x = residual.reshape(-1, self.d_in)
        left_logits = self.left_router(flat_x)
        right_logits = self.right_router(flat_x)
        expert_logits = (
    ...
    def _encode_fixed_expert_stage(
        self,
        residual: Tensor,
        selected_expert_idx: Tensor,
        selected_probs: Tensor,
    ...
    def _encode_stacked(
        ...
        residual = x_centered - stage1_out
        # Reuse one PK-routed expert set across both residual passes so the router
        # cost stays flat while the hard-token cleanup gets a second chance.
        selected_expert_idx, selected_probs = self._select_expert_set(residual)
        stage2a_acts, stage2a_indices, stage2a_full = self._encode_fixed_expert_stage(
            residual,
            selected_expert_idx,
            selected_probs,
            topk=self.stage2a_k,
        ...
        residual_2 = residual - stage2a_out
        stage2b_acts, stage2b_indices, stage2b_full = self._encode_fixed_expert_stage(
            residual_2,
            selected_expert_idx,
            selected_probs,
            topk=self.stage2b_k,
    ...
    def encode(self, x: Tensor) -> EncoderOutput:
        x_centered = x - self.b_dec
        combined_acts, combined_indices, combined_full = self._encode_stacked(
            x_centered
        )
        return EncoderOutput(combined_acts, combined_indices, combined_full)
...
class SharedLowRankCascadeProductKeyExpertJumpReLUSparseCoder(
    SharedLowRankProductKeyExpertJumpReLUSparseCoder
):
    """Low-rank shared coarse branch plus ordered two-expert PK residual cleanup."""

    architecture_name = "shared_lowrank_cascade_product_key_expert_jumprelu"
    ...
    def _select_ordered_expert_pair(self, hidden: Tensor) -> tuple[Tensor, Tensor]:
        left_logits = self.left_router(hidden)
        right_logits = self.right_router(hidden)
        expert_logits = (
            left_logits[:, self.pair_left_index]
    ...
    def _encode_cascade(
        ...
        residual = x_centered - stage1_out
        selected_expert_idx, selected_probs = self._select_ordered_expert_pair(hidden)
...
class ResidualDualSharedProductKeyExpertResidualSparseCoder(
    ResidualDualSharedProductKeyExpertJumpReLUSparseCoder
):
    """Two shared residual stages, PK cleanup, then tiny global residual cleanup."""

    architecture_name = "residual_dual_shared_product_key_expert_residual"
    ...
    def __init__(
        ...
        *,
        ...
        if self.stage2_k < 2:
            raise ValueError(
                f"{self.architecture_name} requires stage2_k >= 2 so routed "
                f"expert cleanup keeps at least one slot after reserving a "
                f"global residual slot; got stage2_k={self.stage2_k}"
            )
    ...
    def encode(self, x: Tensor) -> EncoderOutput:
        ...
        residual_3 = residual_2 - stage2_out
        stage3_acts, stage3_indices, stage3_full = self._encode_residual_stage(
            residual_3
        )

        combined_acts = torch.cat(
            (stage1a_acts, stage1b_acts, stage2_acts, stage3_acts), dim=-1
        )
        combined_indices = torch.cat(
            (stage1a_indices, stage1b_indices, stage2_indices, stage3_indices), dim=-1
        )
        combined_full = torch.cat(
            (stage1a_full, stage1b_full, stage2_full, stage3_full), dim=-1
        )
        return EncoderOutput(combined_acts, combined_indices, combined_full)
...
class LowRankProductKeyExpertJumpReLUSparseCoder(SparseCoder):
    """Low-rank trunk plus product-key-routed JumpReLU expert cleanup."""
    ...
    def __init__(
        ...
        *,
        ...
        if cfg.k > self.active_experts * self.latents_per_expert:
            raise ValueError(
                "lowrank_product_key_expert_jumprelu requires "
                "k <= active_experts * latents_per_expert, "
                f"got k={cfg.k}, active_experts={self.active_experts}, "
                f"latents_per_expert={self.latents_per_expert}"
            )
