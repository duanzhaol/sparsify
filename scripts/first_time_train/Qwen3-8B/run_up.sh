...
BASE_CMD="torchrun --nproc_per_node 4 --master_port 29501 -m sparsify \
      ...
      --split train \
      --wandb_project sparsify_sweep_1222_upproj \
      --ctx_len 2048 \
      ...
      --elbow_threshold_path ./thresholds_up.json \
      --max_tokens 25000000 \
      --exceed_alphas 0.05 0.10 0.20 0.50 1.0 2.0"

# Run experiments with different hookpoints and ports
$BASE_CMD --hookpoints "layers.[0-6].mlp.up_proj"
$BASE_CMD --hookpoints "layers.[7-13].mlp.up_proj"
$BASE_CMD --hookpoints "layers.[14-20].mlp.up_proj"
$BASE_CMD  --hookpoints "layers.[21-27].mlp.up_proj"
$BASE_CMD  --hookpoints "layers.[28-34].mlp.up_proj"