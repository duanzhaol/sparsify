#!/usr/bin/env python3
"""
Export expert_jumprelu SAE checkpoints into standalone LUT bundles.
...
The exporter lives under `scripts/export/` because expert JumpReLU checkpoints
use a dedicated single-router runtime bundle schema.
"""
...
def load_expert_checkpoint(checkpoint_dir: Path) -> dict[str, Any]:
    with open(checkpoint_dir / "cfg.json", "r") as f:
        config = json.load(f)

    if config.get("architecture") != "expert_jumprelu":
        raise ValueError(f"{checkpoint_dir} is not an expert_jumprelu checkpoint")
    ...
    return {
        "router_weight": tensors["router.weight"],
        "router_bias": tensors["router.bias"],
...
def _architecture_config(checkpoint: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "router_out_dim": int(checkpoint["router_weight"].shape[0]),
    }
...
def export_projection_layer(
    *,
    ...
    checkpoint = load_expert_checkpoint(checkpoint_dir)
    ...
    bundle = {
        "router_weight": checkpoint["router_weight"],
        "router_bias": checkpoint["router_bias"],
    ...
    *,
    ...
    return {
        ...
        "creation_info": {
            ...
            "script": "scripts/export/export_expert_jumprelu_lut.py",
        },
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Export expert_jumprelu checkpoints to standalone LUT "
    ...
    parser.add_argument(
        ...
        type=str,
        help=(
            "Checkpoint root containing expert_jumprelu qproj/upproj "
